...
def setup_snapshot_image_grid(training_set, random_seed=0):
    ...
    gw = np.clip(2048 // training_set.image_shape[2], 7, 32) #training_set.image_shape[0] = 512
    gh = np.clip(1080 // training_set.image_shape[1], 4, 32) #training_set.image_shape[1] = 540
...
def gen_sample_using_conditioning_params(G_ema, conditioning_params, z, c, truncation_psi=1, truncation_cutoff=None, neural_rendering_resolution=None, update_emas=False, cache_backbone=False, use_cached_backbone=False, **synthesis_kwargs):
    ...
    return G_ema.synthesis(ws, c, update_emas=update_emas, neural_rendering_resolution=neural_rendering_resolution, cache_backbone=cache_backbone, use_cached_backbone=use_cached_backbone, **synthesis_kwargs)
#----------------------------------------------------------------------------
...
def training_loop(
    ...
    if rank == 0:
        ...
        G_orig = copy.deepcopy(G) # for clip loss
    loss = dnnlib.util.construct_class_by_name(device=device, G_orig=G_orig, G=G, D=D, augment_pipe=augment_pipe, **loss_kwargs) # subclass of training.loss.Loss
    
    phases = []
    for name, module, opt_kwargs, reg_interval in [('G', G, G_opt_kwargs, G_reg_interval)]:
        if reg_interval is None: 
            if name=='G': # G의 parameter만 update하기 위해서
                opt = dnnlib.util.construct_class_by_name(
                    params=list(module.backbone.synthesis.parameters())
                        +list(module.superresolution.parameters()), **opt_kwargs) 
                #opt = dnnlib.util.construct_class_by_name(
                #    params=module.backbone.synthesis.parameters(), **opt_kwargs) 
            else:
            ...
            if name=='G': # G의 parameter만 update하기 위해서
                opt = dnnlib.util.construct_class_by_name(
                    params=list(module.backbone.synthesis.parameters())
                        +list(module.superresolution.parameters()), **opt_kwargs) 
                #opt = dnnlib.util.construct_class_by_name(params=module.backbone.synthesis.parameters(), **opt_kwargs) 
            else:
    ...
    if rank == 0:
        ...
        save_image_grid(images, os.path.join(run_dir, f'real.png'), drange=[-1,1], grid_size=grid_size)
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            ...
            all_gen_z = torch.randn([len(phases) * batch_size, G.z_dim], device=device) #(16,512)
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            for real_img, real_c, gen_z, gen_c in zip(phase_real_img, phase_real_c, phase_gen_z, phase_gen_c):
                ...
                neutral=neutral, target=target, clip_loss_models=clip_loss_models, clip_model_weights=clip_model_weights, id_loss=id_loss)
        ...
        done = (cur_nimg >= total_kimg * 1000) # 이 부분 바꾸기
        if (not done) and (cur_tick != 0) and (cur_nimg < cur_tick*1000): #이 부분 바꾸기
            continue
        ...
        if cur_tick % image_snapshot_ticks == 0:
            ...
            save_image_grid(images, os.path.join(run_dir, f'fakes{cur_nimg//10:06d}.png'), drange=[-1,1], grid_size=grid_size)
            save_image_grid(images_raw, os.path.join(run_dir, f'fakes{cur_nimg//10:06d}_raw.png'), drange=[-1,1], grid_size=grid_size)
            save_image_grid(images_depth, os.path.join(run_dir, f'fakes{cur_nimg//10:06d}_depth.png'), drange=[images_depth.min(), images_depth.max()], grid_size=grid_size)
        ...
        if cur_tick % network_snapshot_ticks == 0:
            ...
            snapshot_pkl = os.path.join(run_dir, f'network-snapshot-{cur_nimg//10:06d}.pkl')
            if rank == 0:
