...
class StyleGAN2Loss(Loss):
    ...
    def run_G_orig(self, z, c, swapping_prob, neural_rendering_resolution, update_emas=False):
        ...
        ws = self.G_orig.mapping(z, c_gen_conditioning, update_emas=update_emas)
        if self.style_mixing_prob > 0:
            with torch.autograd.profiler.record_function('style_mixing'):
                cutoff = torch.empty([], dtype=torch.int64, device=ws.device).random_(1, ws.shape[1])
                cutoff = torch.where(torch.rand([], device=ws.device) < self.style_mixing_prob, cutoff, torch.full_like(cutoff, ws.shape[1]))
                ws[:, cutoff:] = self.G_orig.mapping(torch.randn_like(z), c, update_emas=False)[:, cutoff:]
        gen_output = self.G_orig.synthesis(ws, c, neural_rendering_resolution=neural_rendering_resolution, update_emas=update_emas)
    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, gain, cur_nimg, neutral, target, clip_loss_models, clip_model_weights, id_loss):
        ...
        if phase in ['Gmain', 'Gboth']:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_img_orig, _gen_ws_orig = self.run_G_orig(gen_z, gen_c, swapping_prob=swapping_prob, neural_rendering_resolution=neural_rendering_resolution)
                ...
                c_loss = torch.sum(torch.stack([clip_model_weights[model_name] * clip_loss_models[model_name](gen_img_orig['image'], neutral, gen_img['image'], target) 
                                                for model_name in clip_model_weights.keys()]))
                i_loss = id_loss(gen_img['image'], gen_img_orig['image'])[0]
